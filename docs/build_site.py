...
/* Clears the sticky brand bar, so a jump from the contents rail does not
   land with the heading hidden underneath it. */
section { scroll-margin-top:4.5rem; }
.fn-list, details.fn { scroll-margin-top:4.5rem; }
h2.sec {
...
.hero .brand-logo { height:92px; margin-bottom:18px; }
/* Both variants are in the markup and one is hidden. Every rule below is
   written at the same specificity as the sizing rule, because a bare
   `.lg-dark { display:none }` loses to `.brand-logo img` and leaves both
   showing, stacked. */
.brand-logo img { display:none; height:100%; width:auto; }
.brand-logo img.lg-light { display:block; }
@media (prefers-color-scheme: dark) {
  .brand-logo img.lg-light { display:none; }
  .brand-logo img.lg-dark  { display:block; }
}
:root[data-theme="dark"]  .brand-logo img.lg-light { display:none; }
:root[data-theme="dark"]  .brand-logo img.lg-dark  { display:block; }
...
<div id="brandbar">
  <span class="brand-logo">
    <img class="lg-light" src="assets/logo-light.svg" alt="">
    <img class="lg-dark" src="assets/logo-dark.svg" alt="">
  </span>
  <span class="bb-text">ANOVA <b>Methods</b></span>
  <span class="bb-spacer"></span>
  <a class="bb-link" href="#install">Installation</a>
  <a class="bb-link" href="#psystats">Functions</a>
  <a class="bb-link" href="#citing">Citing</a>
</div>
